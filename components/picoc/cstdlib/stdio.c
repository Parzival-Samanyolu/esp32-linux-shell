...
#include "../picoc_esp32.h"   /* picoc_emit(): route stdout/stderr to the shell socket */
...
void StdioOutPutc(int OutCh, StdOutStream *Stream)
{
    if (Stream->FilePtr != NULL) {
        /* stdout/stderr go to the shell socket; other files use the VFS */
        if (Stream->FilePtr == stdout || Stream->FilePtr == stderr)
            picoc_emit((char)OutCh);
        else
...
/* stdout/stderr -> shell socket; other FILE* -> real fwrite. Returns count. */
static int StdioEmitOrFile(StdOutStream *Stream, const char *s, int n)
{
    if (Stream->FilePtr == stdout || Stream->FilePtr == stderr) {
        for (int k = 0; k < n; k++) picoc_emit(s[k]);
    } else {
        fwrite(s, 1, n, Stream->FilePtr);
    }
    return n;
}
...
{
    if (Stream->FilePtr != NULL)
        { char b[80]; int n = snprintf(b, sizeof(b), Format, Value); Stream->CharCount += StdioEmitOrFile(Stream, b, n); }
...
void StdioFprintfLong(StdOutStream *Stream, const char *Format, uint64_t Value) {
    ...
    if (Stream->FilePtr != NULL)
        { char b[80]; int n = snprintf(b, sizeof(b), PlatformFormat, Value); Stream->CharCount += StdioEmitOrFile(Stream, b, n); }
    else if (Stream->StrOutLen >= 0) {
#ifndef WIN32
